...
describe('subscriptions pause (e2e)', () => {
  let app: INestApplication<App>;
  let moduleRef: TestingModule;
  let pool: Pool;
  let redis: Redis;
  const runId = `t50-${process.pid}-${Date.now()}`;
  ...
  const pause = (subscriptionId: string, key: string) =>
  ...
  it('pauses the subscription and omits the pending intents // T60: @E2E-11 @RF-26', async () => {
    ...
    const response = await pause(subscriptionId, nextKey());

    expect(response.status).toBe(200);
    const body = response.body as SubscriptionDetailResponse;
    expect(body).toMatchObject({ id: subscriptionId, status: 'PAUSED' });
    ...
    expect(persisted.rows[0].status).toBe('PAUSED');
  });

  it('omits a RETRY_PENDING intent with the pause reason', async () => {
    const subscriptionId = await createSubscription();
    await createIntent(
      subscriptionId,
      '2026-05-10',
    ...
    expect(response.status).toBe(200);
    expect((response.body as SubscriptionDetailResponse).status).toBe('PAUSED');
  ...
  it('keeps an IN_FLIGHT attempt and lets it finish while PAUSED // T60: @E2E-11 @RF-26', async () => {
    const subscriptionId = await createSubscription();
    ...
    expect(afterBody.status).toBe('PAUSED');
  ...
  it('replays the settled response for a repeated idempotency key', async () => {
    ...
    const first = await pause(subscriptionId, key);
    const second = await pause(subscriptionId, key);
  ...
  it('does not change an existing pause when called with a new key', async () => {
    const subscriptionId = await createSubscription();

    const first = await pause(subscriptionId, nextKey());
    const second = await pause(subscriptionId, nextKey());

    expect(first.status).toBe(200);
    expect(second.status).toBe(200);
    const firstBody = first.body as SubscriptionDetailResponse;
    const secondBody = second.body as SubscriptionDetailResponse;
    expect(secondBody.status).toBe('PAUSED');
    expect(secondBody.billingIntents).toEqual(firstBody.billingIntents);
  });

  it('returns 409 INVALID_TRANSITION for a CANCELLED subscription', async () => {
  ...
  it('rejects a malformed subscription id with 400', async () => {
    const response = await pause('not-a-uuid', nextKey());
